...
export SYSROOT=/opt/amiga #$TARGET
export PATH=$PATH:${SYSROOT}/bin
export M68K_CPU="-m68080 -mhard-float"
...
cmake .. -G"Unix Makefiles" -DM68K_TOOLCHAIN_PATH=/opt/amiga -DCMAKE_TOOLCHAIN_FILE=../../../CMake/amiga/m68k-amigaos.cmake.simple -DCMAKE_INSTALL_PREFIX=${SYSROOT} -DM68K_CPU=68080 -DM68K_FPU=hard -DM68K_COMMON="${M68K_COMMON} -O3 -fno-exceptions -w -noixemul -DBIG_ENDIAN -DAMIGA -fpermissive -std=c++14"
...
cd libSDL12-master
make PREFX=${SYSROOT} PREF=${SYSROOT} CPU=68080 -j$(getconf _NPROCESSORS_ONLN)
...
make CPU=68080 -j$(getconf _NPROCESSORS_ONLN)
make install
cd ..
...
cmake .. -G"Unix Makefiles" -DM68K_TOOLCHAIN_PATH=/opt/amiga -DCMAKE_TOOLCHAIN_FILE=../../../CMake/amiga/m68k-amigaos.cmake.simple -DCMAKE_INSTALL_PREFIX=${SYSROOT} -DUNIX=1 -DM68K_CPU=68080 -DM68K_FPU=hard -DM68K_COMMON="${M68K_COMMON}"
cmake --build . --config Release --target install -- -j$(getconf _NPROCESSORS_ONLN)
cd ../..
...
cd SDL_ttf-SDL-1.2
./autogen.sh
SDL_LIBS='-lSDL -ldebug' SDL_CFLAGS="-I${SYSROOT}/include/SDL -noixemul"  CFLAGS="${M68K_CFLAGS}" CXXFLAGS="${M68K_CXXFLAGS}" FT2_CFLAGS="-I${SYSROOT}/include/freetype2" FT2_LIBS="-lfreetype -lzlib -lpng16 -lz" ./configure --disable-shared --enable-static --host=${TARGET} --prefix=${SYSROOT}
make CPU=68080 -j$(getconf _NPROCESSORS_ONLN)
make install
